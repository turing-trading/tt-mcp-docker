...
async def call_tool(
    ...
    if name == "list_containers":
        ...
        containers = docker_client.containers.list(all=args.all)
        result = [
            {
                "id": c.id,
                "name": c.name,
        ...
        container.start()
        result = {"status": "started", "id": container.id}

    elif name == "stop_container":
        args = ContainerActionInput.model_validate(arguments)
        container = docker_client.containers.get(args.container_id)
        container.stop()
        result = {"status": "stopped", "id": container.id}
        ...
        result = [{"name": vol.name, "driver": vol.attrs["Driver"]} for vol in volumes]
